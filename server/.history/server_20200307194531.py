   ...
   if request.method == 'POST':
      ...
      for row in df.index:
          for title in df.columns:
            df.at[row, title] = unidecode.unidecode(str(df.at[row, title]))
      titles.df = df
      columns.df = df
      rc.Reader.df = df
      comparison.originalDf = df
      comparison.excelType = excelType
      ret = {"filename": name, "data": titles.getTitlesJson(excelType)}
